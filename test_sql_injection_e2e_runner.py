#!/usr/bin/env python3
"""E2E test for SQL injection protection - Test Runner Version"""
...
def run_test():
    ...
    screenshot_dir = f"{base_path}/agents/a7dd3c03/e2e_test_runner_1_0/img/sql_injection"
    ...
    # Note: Skipping dependency check - will fail during browser launch if deps missing
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                screenshot_path = f"{screenshot_dir}/02_malicious_query_input.png"
                page.screenshot(path=screenshot_path)
                test_result["screenshots"].append(screenshot_path)
                ...
                if not error_message.is_visible():
                    ...
                    test_result["error"] = "CRITICAL FAILURE (Step 7): No security error message displayed. SQL injection attempt was NOT blocked!"
                    ...
                    if "security" not in error_text.lower() and "error" not in error_text.lower():
                        test_result["status"] = "failed"
                        test_result["error"] = f"CRITICAL FAILURE (Step 7): Error message does not contain security-related text. Found: {error_text}"
                        print(f"FAILED: {test_result['error']}")
                ...
                if not tables_section.is_visible():
                    ...
                    if tables_section.count() == 0:
                        test_result["status"] = "failed"
                        test_result["error"] = "FAILURE (Step 9): Failed to find Available Tables section"
                        print(f"FAILED: {test_result['error']}")
                else:
                    ...
                    if "users" not in page_content.lower():
                        test_result["status"] = "failed"
                        test_result["error"] = "CRITICAL FAILURE (Step 9): Users table not found! Database may have been damaged by SQL injection!"
                        print(f"FAILED: {test_result['error']}")
                    else:
        ...
        test_result["status"] = "failed"
        test_result["error"] = f"Unexpected error during test execution: {str(e)}"
        print(f"FAILED: {test_result['error']}")
